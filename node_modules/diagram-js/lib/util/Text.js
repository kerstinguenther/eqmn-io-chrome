...
function parseMargin(margin) {
	if (isObject(margin)) {
	    return assign({ top: 0, left: 0, right: 0, bottom: 0 }, margin);
	  } else {
	    return {
	      top: margin,
	      left: 0,
...
Text.prototype.createText = function(parent, text, options) {
  ...
  var box = merge({}, this._config.size, options.box || {}),
      ...
      padding = parsePadding(options.padding !== undefined ? options.padding  : this._config.padding),
  	  margin = parseMargin(options.margin || this._config.margin);		// Custom
  
  /* START Custom */
  if(options.box && options.box.type.indexOf("eqmn") != -1) {
	  box.width = this._config.size.width;
  ...
  switch (align.vertical) {
    case 'middle':
      y = (box.height - totalHeight) / 2 - layouted[0].height / 4; 	// Custom
      break;

    default:
      y = padding.top + margin.top; // Custom
  }
  ...
  forEach(layouted, function(line) {
	...
	/* START Custom */
	if(margin.top >= 0) {
		y += line.height;	// default (without custom modifications)
	} else {
		y -= line.height;
	}
	/* END Custom */
    ...
    switch (align.horizontal) {
      ...
      default:
        // aka center
        // custom: - (box.width/2 - options.box.width/2)
        x = Math.max(((maxWidth - line.width) / 2 + padding.left), 0) - (box.width/2 - options.box.width/2);
    }
